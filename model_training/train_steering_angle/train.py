...
class Trainer:
    ...
    def _build_training_ops(self, model, learning_rate):
        ...
        loss = tf.reduce_mean(tf.square(tf.subtract(model.y_,model.y))) + \
               tf.add_n([tf.nn.l2_loss(v) for v in train_vars]) * self.l2_norm_const   # Mean-Squared Error + L2 loss * lambda
    ...
    def train(self, epochs, batch_size):
        for epoch in range(epochs):
            self.train_one_epoch(epoch, batch_size)
...
if __name__ == "__main__":
    LOGDIR='save'                   #model_training/train_steering_angle/
    LOGS_PATH='logs'                #model_training/train_steering_angle/
    EPOCHS=30
    BATCH_SIZE=100
    ...
    print("Run the command line:\n"
          "--> tensorboard --logdir=logs "                  #model_training/train_steering_angle/
