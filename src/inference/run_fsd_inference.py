...
#Disable tensorflow v2 behaviour
    ...
    def __init__(self, model_path: str):
        ...
        if not os.path.exists(model_path + ".index"):
            raise FileNotFoundError(
                f"TensorFlow checkpoint not found.\n"
                f"Expected: {model_path}.index"
            )
...
class ImageSegmentation:
    ...
    # parallel processing performing 2 task simultaneously -> lane and object detection

    def process(self, img: np.ndarray, alpha: float = 0.5) -> np.ndarray:
        overlay = np.zeros_like(img)

        with concurrent.futures.ProcessPoolExecutor() as executor:
            future_lane = executor.submit(
                self.lane_model.predict,
                img,
                conf=0.5
            )
        ...
        print("DEBUG overlay type:", type(overlay))
        print("DEBUG overlay shape:", overlay.shape)
    ...
    '''
        #return cv2.addWeighted(overlay,alpha,lane_results,1-alpha,0)
...
class SelfDrivingCarSimulator:
    def __init__(self,steering_model: SteeringAnglePredictor,segmentation_model: ImageSegmentation, data_path:str, img_path:str):
        ...
        if self.img is None:
            raise FileNotFoundError(f"Could not load steering wheel image: {img_path}")
...
# Usage Examples
    ...
    #image_segmentation= ImageSegmentation("saved_models/lane_segmentation_model/best_yolo11_lane_segementation.pt", "saved_models/object_detection_model/yolo11n-seg.pt")
...
if __name__ == "__main__":
    ...
    print("LANE MODEL:", lane_model_path)
    print("LANE EXISTS:", os.path.exists(lane_model_path))
    print("OBJECT MODEL:", object_model_path)
    print("OBJECT EXISTS:", os.path.exists(object_model_path))

    steering_predictor = SteeringAnglePredictor(
        steering_model_path
    )
    ...
    simulator = SelfDrivingCarSimulator(
        steering_predictor,
        image_segmentation,
        os.path.join(PROJECT_ROOT, "data", "driving_dataset"),
        os.path.join(PROJECT_ROOT, "data", "steering_wheel_image.png")
    )
